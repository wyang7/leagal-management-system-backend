...
let currentPage = 1;
// 修改为可变的分页大小，并默认 10
let pageSize = 10; // 默认每页10条，可由下拉选择修改
// 新增全局变量，跟踪当前状态
...
async function showCaseDetailModal(caseId) {
    try {
        ...
        const fmtDate = (d)=> !d?'-':(/\d{4}-\d{2}-\d{2}/.test(d)?d:new Date(d).toLocaleString());
  ...
  : '<div class="text-muted">暂无付款流水</div>'; extHtml=`<div class='row g-2'>
        ...
        if(historyList && historyList.length){
          historyHtml = historyList.map(h=>{ const meta=actionIconMap[h.action]||{icon:'fa-info-circle',color:'#8c8c8c'}; return `<div class='timeline-item mb-2 p-2 rounded border'>
            ...
            <div class='small'><span class='text-muted'>操作人：</span>${h.operatorName||'-'}</div>
        ...
        } else { historyHtml='<div class="text-muted">暂无历史记录</div>'; }
        // 分类块
        ...
        const basicHtml = `<div class='row g-2'>
          ...
          <div class='col-md-6'><span class='text-muted'>处理人：</span>${caseInfo.userName||caseInfo.username||'-'}</div>
 ...
 * 加载案件列表（支持分页）
 */
async function loadCases(pageNum = 1, customPageSize = pageSize, station = '') {
    try {
        // 如果传入的 customPageSize 与当前全局不同（来自选择框），更新全局
        if (customPageSize !== pageSize) {
...
function renderPagination(pageInfo) {
    const { total, pageNum, pageSize: currentPs } = pageInfo;
    const pages = Math.ceil(total / currentPs);
    // 创建分页容器（如果不存在）
    let paginationContainer = document.getElementById('paginationContainer');
    if (!paginationContainer) {
        paginationContainer = document.createElement('div');
        paginationContainer.id = 'paginationContainer';
        paginationContainer.className = 'd-flex flex-column align-items-center mt-4';
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-center gap-3 mb-2 text-secondary">
        <div>共 ${total} 条记录，当前第 ${pageNum}/${pages} 页</div>
        ${buildPageSizeSelector()}
    </div>
    <nav aria-label="案件列表分页">
        <ul class="pagination mb-0">
            <li class="page-item ${pageNum === 1 ? 'disabled' : ''}">
                <a class="page-link" href="#" onclick="loadCases(${pageNum - 1}, ${currentPs})" aria-label="上一页"><span aria-hidden="true">&laquo;</span></a>
            </li>`;
    // 添加第一页按钮（当当前页不在前5页时）
    if (startPage > 1) {
        paginationHtml += `<li class="page-item"><a class="page-link" href="#" onclick="loadCases(1, ${currentPs})">1</a></li>`;
    ...
    // 下一页按钮
    paginationHtml += `<li class="page-item ${pageNum === pages ? 'disabled' : ''}"><a class="page-link" href="#" onclick="loadCases(${pageNum + 1}, ${currentPs})" aria-label="下一页"><span aria-hidden="true">&raquo;</span></a></li></ul></nav>`;
...
function buildPageSizeSelector() {
    ...
    return `<div class="d-flex align-items-center gap-1">
        <label for="pageSizeSelect" class="form-label mb-0 small text-muted">每页</label>
        <select id="pageSizeSelect" class="form-select form-select-sm" style="width:auto;">
            ${options.map(o => `<option value='${o}' ${o === pageSize ? 'selected' : ''}>${o}</option>`).join('')}
        </select>
...
function bindPageSizeChange() {
    ...
    sel.onchange = function() {
        ...
        pageSize = val; // 更新全局变量
        loadCases(1, pageSize, currentStation); // 重载第一页
    };
}
 ...
 * 根据状态筛选案件（支持分页）
 */
async function filterCases(status, pageNum = 1, customPageSize = pageSize) {
    try {
        ...
        loadCases(currentPage, customPageSize, currentStation);
...
// 在文件靠后位置，批量操作等函数之后，增加三个案件操作弹窗：反馈、延期、退回
...
function showPreFeedbackModal(caseId) {
            ...
            <div class="modal-content ant-card ant-card-bordered" style="border-radius:10px;box-shadow:0 4px 16px #e6f7ff;">
                ...
                <div class="modal-body" style="background:#fafcff;">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="preFeedbackContent" class="form-control" rows="4" placeholder="请输入反馈内容"></textarea>
                    </div>
                </div>
                <div class="modal-footer" style="border-top:1px solid #f0f0f0;">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">取消</button>
                    <button type="button" class="btn btn-primary" onclick="submitPreFeedback()">提交反馈</button>
                </div>
            </div>
        </div>
    </div>`;
    ...
    modal.show();
    document.getElementById('preFeedbackModal').addEventListener('hidden.bs.modal', () => temp.remove());
}

async function submitPreFeedback() {
    const caseId = document.getElementById('preFeedbackCaseId').value;
    const content = document.getElementById('preFeedbackContent').value.trim();
    if (!content) {
        alert('请填写反馈内容');
        return;
    }
    try {
        ...
        const modalEl = document.getElementById('preFeedbackModal');
        if (modalEl) {
...
function showDelayModal(caseId) {
            ...
            <div class="modal-content ant-card ant-card-bordered" style="border-radius:10px;box-shadow:0 4px 16px #e6f7ff;">
                ...
                <div class="modal-body" style="background:#fafcff;">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="delayReason" class="form-control" rows="4" placeholder="请输入延期原因"></textarea>
                    </div>
                </div>
                <div class="modal-footer" style="border-top:1px solid #f0f0f0;">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">取消</button>
                    <button type="button" class="btn btn-primary" onclick="submitDelay()">提交延期</button>
                </div>
            </div>
        </div>
    </div>`;
    ...
    modal.show();
    document.getElementById('delayModal').addEventListener('hidden.bs.modal', () => temp.remove());
}
...
function showReturnCaseModal(caseId) {
            ...
            <div class="modal-content ant-card ant-card-bordered" style="border-radius:10px;box-shadow:0 4px 16px #e6f7ff;">
                ...
                <div class="modal-body" style="background:#fafcff;">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="returnReason" class="form-control" rows="4" placeholder="请输入退回原因"></textarea>
                    </div>
                </div>
                <div class="modal-footer" style="border-top:1px solid #f0f0f0;">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">取消</button>
                    <button type="button" class="btn btn-primary" onclick="submitReturnCase()">确认退回</button>
                </div>
            </div>
        </div>
    </div>`;
    ...
    temp.innerHTML = modalHtml;
    document.body.appendChild(temp);
    const modal = new bootstrap.Modal(document.getElementById('returnCaseModal'));
    modal.show();
    document.getElementById('returnCaseModal').addEventListener('hidden.bs.modal', () => temp.remove());
}

async function submitReturnCase() {
    ...
    try {
        ...
        const modalEl = document.getElementById('returnCaseModal');
        if (modalEl) {
            const modal = bootstrap.Modal.getInstance(modalEl);
            modal && modal.hide();
        }
