...
async function showmyCaseDetailModal(caseId) {
    try {
        ...
        const [caseInfoResponse, historyList, paymentFlowsResp, complaintFilesResp] = await Promise.all([
            request(`/case/detail/${caseId}`),
            request(`/case/history/${caseId}`).catch(() => []),
            request(`/case/case-payment-flow/list?caseId=${caseId}`, 'GET'),
            request(`/case/complaint-file/list/${caseId}`).catch(() => [])
        ]);
        ...
        const complaintFiles = complaintFilesResp && complaintFilesResp.data ? complaintFilesResp.data : (Array.isArray(complaintFilesResp) ? complaintFilesResp : []);
            ...
            complaintFilesHtml = complaintFiles.map((file, idx) => {
                ...
                return `<div class='border rounded p-2 mb-2 d-flex align-items-start gap-3'>
                    <div class='flex-shrink-0'>
                        <img src='${imgUrl}' class='complaint-thumbnail'
                             style='width:100px;height:100px;object-fit:cover;cursor:pointer;border-radius:4px;border:1px solid #eee;'
